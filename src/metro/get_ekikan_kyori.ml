...
let get_ekikan_kyori eki1 eki2 tree = List.assoc eki2 (Tree.search tree eki1)

let global_ekikan_tree = inserts_ekikan Tree.empty global_ekikan_list

(* test *)
let test1 = get_ekikan_kyori "表参道" "乃木坂" global_ekikan_tree = 1.4
let test2 = get_ekikan_kyori "国会議事堂前" "赤坂" global_ekikan_tree = 0.8
